...
from ((`visit` `v` left join `visit_type` `vtype` on ((`vtype`.`visit_type_id` = `v`.`visit_type_id`)))
         left join `visit_attribute` `vsattglo` on ((`vsattglo`.`visit_id` = `v`.`visit_id`)))
where ((`v`.`voided` = 0) and (`vtype`.`name` = 'Dosing') and (`vsattglo`.`attribute_type_id` = 3));
